...
@section('title', 'Kelas - Akadia')
@section('menuKelas', 'active')

@section('content')
<div class="container py-4">
    <div class="d-flex justify-content-end mb-4">
        <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#tambahkelas"><b>Tambah Kelas</b></button>
    </div>
    ...
    <div class="row g-4">
        @forelse ($kelas as $item)
            <div class="col-lg-4 col-md-6 mb-3">
                <div class="card shadow-sm text-center border-light rounded">
                    <div class="card-body">
                        <h5 class="card-title text-primary"><b>{{ $item->pelajaran }}</b></h5>
                        <p class="card-text text-muted">{{ $item->idkelas }}</p>
                        <div class="d-flex justify-content-between">
                            <a href="{{ route('siswa.kelas.masuk', $item->id) }}" class="btn btn-outline-primary">Masuk</a>
                            <button type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#keluarKelasModal{{ $item->id }}">Keluar</button>
                        </div>
                    </div>
                </div>
            </div>
